...
void next() {
  ...
  // outer loop to ignore unnecessary thing
  while ((token = *src)) {
    ++src;
...
void expression(int level) {
  ...
  if (token == Num) {
    ...
    *++text = EQ;
    ...
    if (token == Num) {
      // offline imm number, just neg it
      *++text = IMM;
      *++text = -token_val;
      match(Num);
    } else {
      // need online calc, so mul -1
      *++text = IMM;
      *++text = -1;
      *++text = PUSH;
      expression(Inc);
    ...
    match(token);
    expression(Inc);
    // dup(mv) addr to stack -> load to ax -> inc/dec -> result overide ax ->
    // store back via dup addr
    if (*text == LC) {
...
void function_declaration() {
  ...
  while (current_id[Token]) {
    if (current_id[Class] == Loc) {
      // the variable used in function will shadown outside
      // but resume back after parsing of function_declaration
...
int main(int argc, char **argv) {
  ...
  i = Char;
  while (i <= While) {
    // init it as Id first, then replace Token field with i
    next();
    current_id[Token] = i++;
  }
